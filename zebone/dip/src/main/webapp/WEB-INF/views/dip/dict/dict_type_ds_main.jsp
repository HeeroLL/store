...
<html>
	<head>
		...
		<title>各数据源字典管理</title>
		<script type="text/javascript" src="js/jsp/dip/dict/dict_type_ds_main.js"></script>
				...
				<div class="container">
					...
					<table id="mainTABLE" cellspacing="10" style="width: 100%;">
						...
						<tr height="30px">
							<td>数据字典类型编码：</td>
							<td>
								<input size="25" type="text" name="typeCode" title="字典类型编码" msg="输入格式不正确,不应包含特殊字符 '$\ " reg="['$\\]" validate="string|1-30"/>
							</td>
						</tr>
						<tr>
							<td>数据源：</td>
							<td>
								<select name="dsId" validate="select">
									<option value="">请选择</option>
									<c:forEach items="${list}" var="dsObj">
										<option value="${dsObj.id}" <c:if test="${dsObj.id== dsId}"><c:out value="selected"/></c:if>>${dsObj.pName}</option>
									</c:forEach>
								</select>
							</td>
						</tr>
