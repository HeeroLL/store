...
function checkAll(form) {
	...
	if (isError) {
		//$(".ketchup-error-container").length > 0
		//scrollForm();
		return false;
	}
	return true;
...
var appendError = function (error, element) {
	...
	isError=true;
	error = error.replace("undefined", "\u5185\u5bb9");
	if ($("." + element.attr("name") + "Formerror").length == 0) {
		var errorContainer = $("<div>", {"class":"ketchup-error-container " + element.attr("name") + "Formerror", html:"<div class='tip_top'><img src='js/jquery/images/tip/right_bottom.gif'/></div><div class='wrapper'><div class='wrapper_top'></div><ol>" + error + "</ol><div class='wrapper_btm'></div></div><div class='tip_btm'><img src='js/jquery/images/tip/right_top.gif'/></div>"});
...
var showError = function (element) {
	...
	errorContainer.css({left:fOffset.left + element.width() - 100, top:t - 40});
	errorContainer.show().animate({width:"150px"});;//errorContainer.animate({ top: fOffset.top - errorContainer.height()});
	errorContainer.mouseover(function(){
		$(this).hide();
	});
	setTimeout(function () {
		$("." + element.attr("name") + "Formerror").slideUp(function(){$(this).css('width','0')});
	}, 3000);
};
...
var scrollForm = function () {
	destination = $(".ketchup-error-container:first").offset().top;
...
var checkCheckbox=function(el){
	...
	if($("div[name='"+n+"']").length==1){
		...
		$("div[name='"+n+"']").each(function(){
				...
				if(div.attr("disabled")==undefined||div.attr("disabled")==false){
					//alert(n+'--'+div.find('input:[name='+n+']').length+'-'+$('input:[name='+n+']').length);
	...
	//window.top.document.title=cnum;
