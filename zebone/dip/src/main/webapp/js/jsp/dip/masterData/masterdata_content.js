...
var loadDetailDataByID=function(id){
	var formDialog1 = $("#showdetail");
    ...
    $.ajax({
        ...
        success: function(res){
            //基本信息和账号信息的加载
            formDialog1.find("input[type='text']").each(function(){
                var name = $(this).attr('name');
                if(name){
                    $(this).val(res[name]);
                }
            });
            formDialog1.find("input[type='hidden']").each(function(){
                var name = $(this).attr('name');
                if(name){
                    $(this).val(res[name]);
                }
            });
            formDialog1.find("select").each(function(){
                var name = $(this).attr('name');
                if(name){
                    $(this).selectOption(res[name]);
                }
            });
            formDialog1.find("textarea").each(function(){
                var name = $(this).attr('name');
                if(name){
                    $(this).val(res[name]);
                }
...
var resetDetailInfo = function(){
	$("#formshowdetail").find("input[type='hidden']").each(function(){
		$(this).val('');
	});
	$("#formshowdetail").find("input[type='text']").each(function(){
		$(this).val('');
	});
	$("#formshowdetail").find("input[type='select']").each(function(){
		$(this).selectOption('');
	});
    $("#formshowdetail").find("textarea").each(function(){
        $(this).val('');
    });
};
