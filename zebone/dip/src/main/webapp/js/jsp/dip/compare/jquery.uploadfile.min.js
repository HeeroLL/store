	...
	b.fn.uploadFile = function(r) {
				...
				.extend(
						{
							...
							dragDropStr : "<span ><b style='font-size:14px;'>可将文件拖入</b></span>",
							abortStr : "Abort",
							cancelStr : "Cancel",
							deletelStr : "Delete",
							doneStr : "Done",
		...
		function i() {
			if (q.afterUploadAll && !g) {
				...
				(function s() {
					if (l.sCounter != 0
							&& (l.sCounter + l.fCounter == l.tCounter)) {
						q.afterUploadAll(l);
						g = false
					} else {
		...
		function k(G, A, t) {
			for ( var B = 0; B < t.length; B++) {
				if (!c(A, G, t[B].name)) {
					if (G.showError) {
						b(
								"<div style='color:red;'><b>" + t[B].name
										+ "</b> " + G.extErrorStr
										+ G.allowedTypes + "</div>").appendTo(
								A.errorLog)
					}
					continue
				}
				...
				D.filename.html(F);
				var u = b("<form style='display:block; position:absolute;left: 150px;' class='"
						+ A.formGroup
						+ "' method='"
						+ G.method
						+ "' action='"
						+ G.url + "' enctype='" + G.enctype + "'></form>");
				u.appendTo("body");
				var w = [];
				w.push(t[B].name);
				m(u, C, D, w, A);
		...
		function p(x, A, C, t) {
			...
			var v = b("<form method='" + C.method + "' action='" + C.url
					+ "' enctype='" + C.enctype + "'></form>");
			...
			if (C.multiple) {
				...
				u = "<input type='file' id='" + z + "' name='" + C.fileName
						+ "' multiple/>"
			}
			...
			y.change(function() {
				...
				if (this.files) {
					...
					if (!c(x, C, H)) {
						if (C.showError) {
							b(
									"<div style='color:red;'><b>" + H + "</b> "
											+ C.extErrorStr + C.allowedTypes
				...
				if (a.fileapi && a.formdata) {
					...
					for ( var G = 0; G < F.length; G++) {
						if (C.showFileCounter) {
							D += x.fileCounter + C.fileCounterStyle + F[G]
									+ "<br>"
						} else {
							D += F[G] + "<br>"
						}
		...
		function o(u, t) {
			...
			this.progressbar = b(
					"<div class='ajax-file-upload-bar " + u.formGroup
							+ "'></div>").appendTo(this.progressDiv);
			this.abort = b(
					"<div class='ajax-file-upload-red " + u.formGroup + "'>"
							+ t.abortStr + "</div>").appendTo(this.statusbar)
					.hide();
			this.cancel = b(
					"<div class='ajax-file-upload-red'>" + t.cancelStr
							+ "</div>").appendTo(this.statusbar).hide();
			this.done = b(
					"<div class='ajax-file-upload-green'>" + t.doneStr
							+ "</div>").appendTo(this.statusbar).hide();
			this.del = b(
					"<div class='ajax-file-upload-red'>" + t.deletelStr
							+ "</div>").appendTo(this.statusbar).hide();
		...
		function m(y, x, t, v, z) {
			...
			var u = {
				...
				beforeSubmit : function(E, B, D) {
					...
					t.statusbar.append("<div style='color:red;'>"
							+ x.uploadErrorStr + "</div>");
					t.cancel.show();
					y.remove();
