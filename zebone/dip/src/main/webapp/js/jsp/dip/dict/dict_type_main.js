...
var loadDictData = function(id){
	...
	$.ajax({
		url:'dict/loadDictTypeStandardInfo.zb',
		type: 'POST',
		cache: false,
		data: eval("({id:'"+id+"'})"),
		dataType: 'json',
...
var saveDictTypeInfo = function(){
	...
	data +="}";
	$.ajax({
		url:'dict/saveDictTypeStandardInfo.zb',
		type: 'POST',
		cache: false,
		data: eval('('+data+')'),
		dataType: 'json',
		success: function(res){
			...
			if(res.success){
				if(!id){
					$("#dictionaryTypeForm").find("input[name='typeId']").val(res.id);
				}
...
$(function(){
	grid = new JGrid({
		...
		dataCol:'',
		checkbox : true,
		dataUrl : 'dict/dictTypeStandardList.zb',
		render : 'grid',
		pageBar : true,
		...
		listeners : {
			...
			curdButtonClick : function(btn) {
				if (btn == 'add') {
					...
					grid.opt.removeUrl = "dict/removeDictTypeStandardByIds.zb";
