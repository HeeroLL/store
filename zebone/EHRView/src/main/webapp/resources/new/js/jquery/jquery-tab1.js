...
JTab.prototype = {
    ...
    addTab: function(opt){
        ...
        if($("#"+options.id).length==0){
            this.tabBar.append("<li id='"+options.id+"' show='0' able='1' ><a  href='javascript:void(0);'><span>"+(options.title.length>11?options.title.substr(0,11)+"...":options.title)+"</span></a></li>");
            if(options.icon){
            ...
            if(options.url){
                ...
                var tb=options.html;
                this.tabBody.append("<div id='"+options.id+"-main' class='tab-body-child''></div>");
                $(tb).appendTo('#'+options.id+'-main');
            }
            if(options.disabled){
    ...
    activeFist:function(){
        this.tabBar.find("li").removeClass("active");
        this.tabBody.find('.tab-body-child').hide();
        this.tabBar.find("li:first").addClass("active").attr('show', '1');
        this.tabBody.find('.tab-body-child:first').show();
    },
    //获取选中的tab
...
(function($) {
    $.fn.jtabs = function(options) {
        var obj=this;//this.get(0)||$('<div />'), options
        obj.height('100%');
        var nav=obj.find('>ul li');
        var bodys=obj.find('>div');
        var t=new JTab({
            renderTo:obj,
            header:false
        });
