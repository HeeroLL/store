  ...
  Autocomplete.prototype = {
    ...
    initialize: function() {
      ...
      if(this.options.valueField){
      	if($('input[name='+this.options.valueField.dataName+']').length==0){
      		$('<input name="'+this.options.valueField.dataName+'" type="hidden"/>').insertBefore(this.el);
      	}
      	var hiddenId=this.options.valueField.dataId?this.options.valueField.dataId:'';
      	if(hiddenId!=''&&$('#'+hiddenId).length==0){
      		$('<input id="'+hiddenId+'" type="hidden"/>').insertBefore(this.el);
      	}
      }
    },
    ...
    getSuggestions: function(q) {
      ...
      var cr, me;
      cr = this.isLocal ? this.getSuggestionsLocal(q) : this.cachedResponse[q];//本地
    ...
    select: function(i) {
      ...
      //多选,点击一项后不关闭提示.只是给选中行添加选中标记.
      	...
      	if(valueField){
	      	//根据input的name传值
	      	if(valueField.dataName){
	      		$('input[name='+valueField.dataName+']').val(getValue.values);
	      	}
	      	//根据input的id传值
	      	if(valueField.dataId){
	      		$('#'+valueField.dataId).val(getValue.values);
	      	}
      	}else{
      		me.el.val(getValue);
      	}
      ...
      if(typeof(selectedValue)=='string'){
	      ...
	      if(valueField){
	      	var value=selectedValue[valueField.dataIndex];
	      	//根据input的name传值
	      	if(valueField.dataName){
	      		$('input[name='+valueField.dataName+']').val(value);
	      	}
	      	//根据input的id传值
	      	if(valueField.dataId){
	      		$('#'+valueField.dataId).val(value);
	      	}
	      }
      }
      if (selectedValue) {
