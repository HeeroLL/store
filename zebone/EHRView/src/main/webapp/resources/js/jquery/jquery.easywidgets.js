...
(function($){
    ...
    if(canDisable){
      ...
      $(s.selectors.widget).each(function(){
        ...
        if(widget.hasClass(s.options.movable)){
          widget.find(s.selectors.header).css('cursor', 'default');//组件失效后
        }
      });
  ...
  $.fn.EasyWidgets.defaults = {
    ...
    behaviour : {
      ...
      // Miliseconds delay between mouseup and drag stop  模块降落时间
      dragRevert : 300,
    ...
    i18n : {
      ...
      cancelEditTitle : '编辑此版块',/*zhangcy 2010-03-04*/
  ...
  function RepositionedWidgets(settings){
    ...
    if($.trim(positions) != ''){
      // Get the widgets places IDs and widgets IDs记录模块的位置和排序
  ...
  function PrepareSortablePlaces(sortableItems, settings){
    ...
    $(s.selectors.places).sortable({
      items: sortableItems,
      containment: 'document',//组件的父容器，可拖动的范围
  ...
  function WidgetsPositionsChange(settings){
    ...
    /*2010-10-08 yanghl add
    var ps = document.getElementById('style').value;
    if(ps == '11' || ps == '13') {
      	var p3 = positions.split("place-3=");
      	if(p3.length>1) {
      		var p3b = p3[0];
      		var p3a = p3[1];
      		var p1 = p3b.split(";place-2=");
      		if(p1.length>1) {
      			var p1b = p1[0]+","+ p3a;
      			positions = p1b+";place-2="+p1[1];
      		}
      	}
      }*/
      //end
  ...
  function AddWidgetCollapseLink(widget, widgetMenu, settings){
    ...
    if(widget.hasClass(s.options.collapsable)){
      ...
      $(link).mousedown(function(e){
        ...
        var haveId = $.trim(widgetId) != '';
        var content = widget.find(s.selectors.content);
        var contentVisible = content.css('display') != 'none';//隐藏正文
        link.blur();
        if(contentVisible){
  ...
  function AddWidgetEditLink(widget, widgetMenu, settings){
    ...
    if(widget.hasClass(s.options.editable)){
      ...
      $(link).mousedown(function(e){
        ...
        link.blur();
        if(false){//editboxVisible更改为始终不可见
          ...
          if(canShow){
            ...
            if($.isFunction(s.callbacks.onEdit)){
              //editPlate(widget.attr('id'));//后加的方法*******************************************
              //s.callbacks.onEdit(link, widget); 原来是这样子的
              s.callbacks.onEdit(widget.attr('id'));
            }
          }
        }
        return false;
          ...
          if(!widget.hasClass(s.options.closeConfirm)
            || confirm(s.i18n.confirmMsg)){
              ...
              if($.isFunction(s.callbacks.onClose)){
                //s.callbacks.onClose(link, widget); zhangcy 修改2010-01-08
                s.callbacks.onClose(widgetId);
              }
          }
        }
        return false;
