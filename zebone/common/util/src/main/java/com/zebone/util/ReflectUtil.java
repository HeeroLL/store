...
public final class ReflectUtil {
     ...
     * @return 对象
     */
    public static Object convertMapToObject(Map<String, ?> map, Class<?> c) {
        ...
        for (Field field : c.getDeclaredFields()) {
            for (Entry<String, ?> entry : map.entrySet()) {
                ...
                // setValue
                setFieldValue(obj, field.getName(), entry.getValue());
            }
        }
        return obj;
    }
     ...
     * @return boolean
     */
    public static boolean hasField(Class<?> c, String filedName) {
        try {
            return c.getDeclaredField(filedName) != null;
        }
        catch (SecurityException e) {
            log.error("Get field failed! Class is " + c.getClass() + ", filed name is " + filedName, e);
            throw new RuntimeException("getMethod", e);
        }
        catch (NoSuchFieldException e) {
            log.warn("Filed name does not exist! Filed name is " + filedName, e);
            return false;
        }
    }
