...
public class ChineseToPinYin {
	 ...
	 * @throws BadHanyuPinyinOutputFormatCombination
	 */
	private static String[] getFullAndForShortSpell(String zhongwen) throws BadHanyuPinyinOutputFormatCombination {
     ...
     * 
     */
    public static String chineseToShortPinyin(String zhongwen)
    {
        if (zhongwen == null)
        {
            return null;
        ...
        try
        {
            ...
            for (int i = 0; i < chars.length; i++)
            {
                ...
                if (pinYin != null)
                {
                    outPut.append(capitalize(pinYin[0])[1]);
                }
                else if (pattern.matcher(String.valueOf(chars[i])).matches())
                {
